...
def _scg_simulate_daubechies(duration=10, length=None, sampling_rate=100, heart_rate=70, respiratory_rate=15, systolic=120, diastolic=80):
    ...
    duration_2 = 1  # Duration of the wave (in seconds)
    sampling_rate = 100.0  # Sampling rate (in Hz)

    # Generate the time axis
    t = np.linspace(0, duration_2, int(duration_2 * sampling_rate), endpoint=False)
    ...
    ## adds leading zeros
    length_2 = np.random.randint(1, 11)
    lead_signal = np.zeros(length_2)
