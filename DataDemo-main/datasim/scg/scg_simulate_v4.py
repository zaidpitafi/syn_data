...
def scg_simulate(
    ...
    if method.lower() in ["simple", "daubechies"]:
        ...
        scg, peaks_1, peaks_2 = _scg_simulate_daubechies(
...
def _scg_simulate_daubechies(duration=10, length=None, sampling_rate=100, heart_rate=70, respiratory_rate=15, systolic=120, diastolic=80):
    ...
    # cardiac_d = np.copy(cardiac_s)
    cardiac_d = scipy.signal.wavelets.daub(ind)*diastolic/80 # change height to 0.3
    cardiac_s = scipy.signal.resample(cardiac_s, 100)
    cardiac_d = scipy.signal.resample(cardiac_d, 100)
    cardiac_s = cardiac_s[0:20]
    cardiac_d = cardiac_d[0:20]
    ...
    # zero_signal = np.random.uniform(min(cardiac_d), max(cardiac_d), distance)
    # zero_signal = np.zeros(distance)
    ## Define Sine Wave
    # Define the parameters of the sine wave
    amplitude = 0.7*np.max(cardiac_s)  # Amplitude of the wave
    cycles = 3
    frequency = 10   # Frequency of the wave (in Hz)
    phase = np.pi/2   # Phase shift of the wave (in radians)
    ...
    # Generate the sine wave using numpy.sin
    y = amplitude * np.sin(2 * np.pi * frequency * t + phase)

    zero_signal = y
    zero_signal = zero_signal[:distance]

    signal_end = np.zeros(500)
    
    ## adds leading zeros
    length = np.random.randint(1, 11)
    lead_signal = np.zeros(length)

    cardiac = np.concatenate([lead_signal, cardiac_s, zero_signal, cardiac_d])
    # cardiac = scipy.signal.resample(cardiac, 100) # fix every cardiac length to 100
    a = cardiac.shape[0]
    if a < 200:
        # cardiac = np.append(cardiac,np.random.uniform(min(cardiac_d), 0.5*max(cardiac_d)))
        # cardiac = np.append(cardiac, 0)
        b = 200 - a 
        cardiac = np.concatenate((cardiac, signal_end[:b]))
    ...
    val_1 = len(lead_signal) + 20
    peak_1_array = find_peaks(scg[:val_1])
    peak_1 = peak_1_array[0][0]
    val_2 = val_1 + distance
    peak_2_array = find_peaks(scg[val_2:val_2+20])
    peak_2 = peak_2_array[0][0] + val_2
