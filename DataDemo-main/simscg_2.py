...
if __name__ == '__main__':
    ...
    for ind in tqdm(range(N)):
        ...
        data, peak_1, peak_2 = nk.scg_simulate(duration=duration, sampling_rate=fs, noise=noise, heart_rate=heart_rate, respiratory_rate=respiratory_rate, systolic=systolic, diastolic=diastolic)
        ## N + 6 size. 6 are [mat_int(here 0 for synthetic data), time_stamp, hr, rr, sbp, dbp]
        simulated_data.append(list(data)+[0]+[ind]+[peak_1]+[peak_2]+[systolic]+[diastolic])

        print('p1:', peak_1, 'p2:', peak_2, 'sp:', systolic, 'dp:', diastolic)
